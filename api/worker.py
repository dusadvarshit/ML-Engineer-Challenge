...
def run_batch_detection(
    ...
    try:
        payloads = [base64.b64decode(payload, validate=True) for payload in encoded_payloads]
        ...
        if len(detections_by_image) != len(payloads):
            raise RuntimeError('Model returned a result count different from the submitted batch.')
        complete_batch_job(
            job_id=job_id,
            model_version=model_version,
            items=[{'detections': [item.model_dump() for item in detections]} for detections in detections_by_image],
        )
        observe_inference(
            task=job.task,
            model=model_version,
        ...
        observe_batch_request(task=job.task, batch_size=len(payloads), outcome='success')
    except Exception as exc:
        observe_batch_request(task=job.task, batch_size=len(encoded_payloads), outcome='error')
        retries = getattr(getattr(self, 'request', None), 'retries', 0)
        if retries < settings.BATCH_TASK_MAX_RETRIES:
            logger.warning('Retrying batch job %s after worker error: %s', job_id, exc)
            raise self.retry(exc=exc, countdown=min(2 ** (retries + 1), 30))
        logger.exception('Batch job %s failed permanently', job_id)
        fail_batch_job(job_id=job_id, error='Batch inference failed. Please retry with a new request.')
