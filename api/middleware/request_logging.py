    ...
    _enqueue_request_log(
        {
            ...
            'error_message': None if response.status_code < 400 else _extract_error_message(response_body),
        }
    )
    return response
...
async def _build_request_payload(request: Request, raw_body: bytes) -> dict[str, Any]:
    ...
    if content_type.startswith('application/x-www-form-urlencoded'):
        payload['form'] = _flatten_query_values(parse_qs(raw_body.decode('utf-8'), keep_blank_values=True))
        return payload
...
def _parse_json_body(raw_body: bytes) -> Any:
    ...
    try:
        ...
        return {'body_preview': _truncate_text(raw_body.decode('utf-8', errors='replace'))}
...
def _serialize_response_payload(*, body: bytes, content_type: str | None) -> dict[str, Any] | None:
