...
class FakeRequest:
    ...
    def __init__(
        ...
        *,
        ...
        route = (
            SimpleNamespace(path=route_path)
...
def test_metrics_response_returns_prometheus_payload(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """The metrics endpoint should expose the generated Prometheus payload."""

    monkeypatch.setattr(
        metrics_module, "generate_latest", lambda: b"metric 1\n"
    )
...
def test_instrument_http_request_records_success_metrics(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        metrics_module, "HTTP_REQUEST_DURATION_SECONDS", duration_histogram
    )
    monkeypatch.setattr(
        metrics_module, "perf_counter", iter([10.0, 10.25]).__next__
    )
    ...
    assert duration_histogram.calls[0][0] == {
        "method": "GET",
        "path": "/health",
    }
...
def test_instrument_http_request_records_exception_metrics(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(metrics_module, "HTTP_REQUESTS_TOTAL", request_counter)
    monkeypatch.setattr(
        metrics_module, "HTTP_REQUEST_EXCEPTIONS_TOTAL", exception_counter
    )
    monkeypatch.setattr(
        metrics_module, "HTTP_REQUEST_DURATION_SECONDS", duration_histogram
    )
    monkeypatch.setattr(
        metrics_module, "perf_counter", iter([20.0, 20.5]).__next__
    )
...
def test_observe_inference_tracks_volume_and_duration(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    duration_histogram = FakeMetric()
    monkeypatch.setattr(
        metrics_module, "MODEL_INFERENCE_REQUESTS_TOTAL", request_counter
    )
    monkeypatch.setattr(
        metrics_module, "MODEL_INFERENCE_DURATION_SECONDS", duration_histogram
    )
...
def test_observe_batch_request_tracks_count_and_size(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    metrics_module.observe_batch_request(
        task="detect", batch_size=4, outcome="success"
    )

    assert batch_counter.calls[0][0] == {
        "task": "detect",
        "outcome": "success",
    }
...
def test_set_dependency_status_updates_gauge(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    metrics_module.set_dependency_status(
        dependency="redis", is_available=False
    )
...
def test_resolve_path_template_prefers_route_path() -> None:
    ...
    path = metrics_module._resolve_path_template(
        FakeRequest(path="/api/v1/detect", route_path="/api/v1/detect")
    )
...
def test_resolve_path_template_falls_back_to_raw_path() -> None:
    ...
    path = metrics_module._resolve_path_template(
        FakeRequest(path="/raw-path", route_path=None)
    )
