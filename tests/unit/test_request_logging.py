...
def test_serialize_response_payload(
    ...
    assert (
        request_logging_module._serialize_response_payload(
            body=body,
...
def test_extract_error_message_prefers_detail_field() -> None:
    ...
    assert (
        request_logging_module._extract_error_message(
...
def test_enqueue_request_log_swallows_broker_errors(
...
def test_persist_request_log_saves_payload(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    worker_module.persist_request_log.run(
        {"request_id": "req-1", "status_code": 200}
    )
