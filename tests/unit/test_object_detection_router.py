...
from api.routers.object_detection import (
    _read_image_bytes,
    batch_inference,
    detect_objects,
)
...
def test_detect_objects_returns_cached_result_without_running_inference(
    mocker,
) -> None:
    ...
    get_detection.assert_awaited_once_with(
        image_bytes, "yolov8n-v1.0.0-best.pt"
    )
...
def test_detect_objects_translates_service_errors(
    mocker, error_type: type[Exception]
) -> None:
...
def test_batch_inference_returns_cached_results_without_running_batch_inference(
    mocker,
) -> None:
    ...
    predict_batch = mocker.patch.object(
        yolo_prediction_service, "predict_batch_from_bytes"
    )

    response = asyncio.run(
        batch_inference(task=InferenceTask.DETECT, files=files)
    )
...
def test_batch_inference_only_runs_yolo_for_cache_misses_and_preserves_order(
    mocker,
) -> None:
...
def test_batch_inference_returns_fresh_results_when_cache_operations_fail(
    mocker,
) -> None:
...
def test_batch_inference_rejects_batches_over_route_limit() -> None:
    ...
    files = [
        _build_upload_file(VALID_IMAGE, f"image-{index}.png")
        for index in range(6)
    ]

    with pytest.raises(
        HTTPException, match="Batch requests support up to 5 images"
    ) as exc_info:
...
def test_batch_inference_rejects_unimplemented_classification_task() -> None:
    ...
    with pytest.raises(
        HTTPException, match="Batch classification is not supported yet"
    ) as exc_info:
...
@pytest.mark.parametrize("error_type", [FileNotFoundError, RuntimeError])
def test_batch_inference_translates_service_errors(
    mocker, error_type: type[Exception]
) -> None:
...
def test_read_image_bytes_rejects_missing_content_type() -> None:
    ...
    with pytest.raises(HTTPException, match="JPEG, PNG, or WebP") as exc_info:
        asyncio.run(
            _read_image_bytes(
                _build_upload_file(VALID_IMAGE, content_type=None)
            )
        )
