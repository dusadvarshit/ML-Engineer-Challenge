...
from api.services.object_detection.retinanet_service import RetinaNetPredictionService
...
def test_resolve_model_path_prefers_sorted_checkpoint_files(tmp_path: Path) -> None:
    ...
    from api.services.object_detection import retinanet_service as retinanet_service_module
...
def test_resolve_model_path_raises_when_no_checkpoint_exists(tmp_path: Path) -> None:
    ...
    retinanet_service_module.settings.RETINANET_MODEL_DIR = tmp_path
    try:
        with pytest.raises(FileNotFoundError, match="No RetinaNet checkpoint found"):
...
def test_predict_batch_normalizes_model_results(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        service,
        "_load_model",
        lambda: (fake_model, lambda image: f"tensor:{image.width}x{image.height}", fake_torch),
    )
...
def test_load_warms_up_loaded_model(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        service,
        "_warm_up",
        lambda model, to_tensor, torch: warmup_calls.append((model, to_tensor, torch)),
    )
...
def test_get_model_version_uses_cached_model_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert service.get_model_version() == "retinanet_resnet50_fpn-v1.0.0-model.pth"
