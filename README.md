...
This repository now includes GitHub-based CI under `.github/workflows/`:

- `ci.yml`
  - runs on pull requests plus pushes to `main`
...
The workflow uses the repository `GITHUB_TOKEN` with:

- `contents: read`

If your default branch is not `main`, update `ci.yml` accordingly.
